  ...
  implements OutAmpWebSocket
{
    ...
    throws IOException
  {
    ...
    try (Writer out = session.getBasicRemote().getSendWriter()) {
      ...
      send(headers, to, methodName, args);
    }
    */
  }
    ...
    throws IOException
  {
    ...
    if (values != null && values.size() > 0) {
      try (Writer out = session.getBasicRemote().getSendWriter()) {
        init(out);
        streamResult(headers, to, qid, values);
      }
    }
    ...
    if (exn != null) {
      try (Writer out = session.getBasicRemote().getSendWriter()) {
        init(out);
        streamError(headers, to, qid, exn);
      }
    }
    else if (isComplete) {
      try (Writer out = session.getBasicRemote().getSendWriter()) {
        init(out);
        streamComplete(headers, to, qid);
      }
    }
    */
  }

...
public class ShortArraySerializer
  extends AbstractJsonArraySerializer<short[]>
{
  private static final L10N L = new L10N(ShortArraySerializer.class);
  
  static final ShortArraySerializer SER = new ShortArraySerializer();
  ...
  public short []read(JsonReaderImpl in)
  {
    ...
    while ((event = in.peek()) != Event.END_ARRAY && event != null) {
      values = addValue(values, i++, (short) in.readLong());
    }
  ...
  private short []addValue(short []values, int i, short value)
  {
    if (values.length <= i) {
      short []newValues = new short[values.length * 2];

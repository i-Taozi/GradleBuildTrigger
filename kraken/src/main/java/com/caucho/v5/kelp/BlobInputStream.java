...
public class BlobInputStream extends InputStream
{
   ...
   */
  public BlobInputStream(RowCursor cursor,
                         ColumnBlob column,
                         int blobOffset,
                         int blobLength,
                         byte []pageBuffer)
  ...
  {
    if (_offset < _tail) {
      return _pageBuffer[_offset++] & 0xff;
    }
    else {
      return -1;
    }
  ...
  public int read(byte []buf, int offset, int length)
    throws IOException
  {
    int sublen = Math.min(_tail - _offset, length);
    
    System.arraycopy(_pageBuffer, _offset, buf, offset, sublen);
